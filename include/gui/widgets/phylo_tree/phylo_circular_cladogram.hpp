#ifndef GUI_WIDGETS_PHY_TREE___PHYLO_CIRCULAR_CLADOGRAM__HPP
#define GUI_WIDGETS_PHY_TREE___PHYLO_CIRCULAR_CLADOGRAM__HPP

/*  $Id: phylo_circular_cladogram.hpp 42903 2019-04-25 15:57:26Z katargir $
 * ===========================================================================
 *
 *                            PUBLIC DOMAIN NOTICE
 *               National Center for Biotechnology Information
 *
 *  This software/database is a "United States Government Work" under the
 *  terms of the United States Copyright Act.  It was written as part of
 *  the author's official duties as a United States Government employee and
 *  thus cannot be copyrighted.  This software/database is freely available
 *  to the public for use. The National Library of Medicine and the U.S.
 *  Government have not placed any restriction on its use or reproduction.
 *
 *  Although all reasonable efforts have been taken to ensure the accuracy
 *  and reliability of the software and data, the NLM and the U.S.
 *  Government do not and cannot warrant the performance or results that
 *  may be obtained by using this software or data. The NLM and the U.S.
 *  Government disclaim all warranties, express or implied, including
 *  warranties of performance, merchantability or fitness for any particular
 *  purpose.
 *
 *  Please cite the author in any work or product based on this material.
 *
 * ===========================================================================
 *
 * Authors:  Bob Falk
 *
 * File Description:
 *
 */

#include <corelib/ncbistd.hpp>
#include <corelib/ncbiobj.hpp>

#include <gui/widgets/phylo_tree/phylo_tree_render.hpp>
#include <gui/widgets/phylo_tree/phylo_tree_ds.hpp>

#include <gui/utils/vect4.hpp>


BEGIN_NCBI_SCOPE

class CGlVboNode;

class NCBI_GUIWIDGETS_PHYLO_TREE_EXPORT CPhyloCircularCladogram
    : public IPhyloTreeRender
{
public:
    CPhyloCircularCladogram();
    CPhyloCircularCladogram(double w, double h);
    ~CPhyloCircularCladogram();
    string  GetDescription(void);

    /// branch length in circular not connected to node distance
    virtual bool SupportsDistanceRendering() const { return true; }
    bool    SupportsRotatedLabels() const { return true; }

    void UseMinimalRotation(bool b) { m_UseMinimalRotation = b; }
    bool GetUseMinimalRotation() const { return m_UseMinimalRotation; }

    virtual double ComputeDistFromPixels(CGlPane& pane,
        const CVect2<TModelUnit>& pixpos1,
        const CVect2<TModelUnit>& pixpos2) const;

    /// Allocate less spaces for labels in circular views since they appear on both sides
    /// of nodes, thus using up more space.
    virtual float   GetLabelViewPct() const { return m_LabelViewPct/2.0f; }

    virtual float GetNodeLabelDist(const CPhyloTreeNode * node) const;

protected:
    /// type of corner
    enum  ECorner {
        eTop,
        eMiddle,
        eBottom
    };

    void  x_Calculate(CPhyloTree* tree);

    void  x_DrawTree(CPhyloTree* tree, 
        vector<float>& color_coords,
        vector<CVect2<float> >& edge_node_coords,
        vector<CVect4<unsigned char> >& edge_node_colors,
        vector<CVect2<float> >& sel_edge_node_coords,
        vector<CVect4<unsigned char> >& sel_edge_node_colors);

    void  x_RenderVbo(CPhyloTreeDataSource& ds);
    void  x_Layout(CPhyloTreeDataSource& ds);
    virtual void x_SetGlRenderOptions(CPhyloTreeDataSource& ds);

    TModelUnit x_GetVerticalSeparationFactor() const { return TModelUnit(2.0); }
    bool x_WideLines();

    /// Center of cladogram
    CVect2<float> m_Center;
    /// If true, labels will only be rotated by amount required to avoid overlap
    /// when rotated labels are in use.
    bool m_UseMinimalRotation;

};

END_NCBI_SCOPE

#endif //GUI_WIDGETS_PHY_TREE___PHYLO_CIRCULAR_CLADOGRAM__HPP

