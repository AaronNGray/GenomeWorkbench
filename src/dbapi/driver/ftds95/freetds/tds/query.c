...
TDS_RCSID(var, "$Id: query.c 554113 2017-12-28 16:13:25Z ucko $");
 ...
 */
const char *
tds_convert_string(TDSSOCKET * tds, TDSICONV * char_conv, const char *s, ssize_t len, size_t *out_len)
{
	char *buf;
...
tds_submit_query_params(TDSSOCKET * tds, const char *query, TDSPARAMINFO * params, TDSHEADERS * head)
{
	...
	if (IS_TDS50(tds->conn)) {
		...
		if (params) {
			/* add on parameters */
            int flags = (tds_dstr_isempty(&params->columns[0]->column_name)
...
tds_submit_execdirect(TDSSOCKET * tds, const char *query, TDSPARAMINFO * params, TDSHEADERS * head)
{
	...
	if (IS_TDS7_PLUS(tds->conn)) {
		...
		for (i = 0; i < params->num_cols; i++) {
			...
			param = params->columns[i];
			/* TODO check error */
            if (TDS_SUCCEED(ret = tds_put_data_info(tds, param, 0)))
...
tds_fix_column_size(TDSSOCKET * tds, TDSCOLUMN * curcol)
{
	...
	case 2:
        ...
        if (size == 0  &&  curcol->column_output)
            min = 8000;
        else if (curcol->on_server.column_type == XSYBNVARCHAR || curcol->on_server.column_type == XSYBNCHAR)
			min = 2;
		else
			min = 1;
...
tds_put_data_info(TDSSOCKET * tds, TDSCOLUMN * curcol, int flags)
{
    ...
    if (IS_TDS50(tds->conn)  &&  (flags & TDS_PUT_DATA_IQ_PARAM) == 0)
...
tds_put_params(TDSSOCKET * tds, TDSPARAMINFO * info, int flags)
{
    ...
    if ( !strcmp(tds->conn->product_name, "Adaptive Server Anywhere") ) {
	...
	for (i = 0; i < info->num_cols; i++) {
        if (TDS_FAILED(tds_put_data_info(tds, info->columns[i], flags))) {
            return TDS_FAIL;
        }
	}
...
tds_submit_rpc(TDSSOCKET * tds, const char *rpc_name, TDSPARAMINFO * params, TDSHEADERS * head)
{
	...
	if (IS_TDS50(tds->conn)) {
        ...
        if (num_params  &&
            TDS_FAILED(tds_put_params(tds, params, TDS_PUT_DATA_USE_NAME))) {
            return TDS_FAIL;
        }
