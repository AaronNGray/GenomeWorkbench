/*  $Id: latlon_tool.cpp 43676 2019-08-14 14:28:05Z asztalos $
 * ===========================================================================
 *
...
IMPLEMENT_DYNAMIC_CLASS( CLatLonTool, wxDialog )

BEGIN_EVENT_TABLE( CLatLonTool, wxDialog )

    EVT_BUTTON( wxID_OK, CLatLonTool::OnClickOk )

    EVT_BUTTON( wxID_CANCEL, CLatLonTool::OnClickCancel )
...
CLatLonTool::CLatLonTool( wxWindow* parent, CSeq_entry_Handle seh, 
...
bool CLatLonTool::Create( wxWindow* parent, wxWindowID id, const wxString& caption, const wxPoint& pos, const wxSize& size, long style )
{
...
CRef<CSeq_table> CLatLonTool::GetValuesTableFromSeqEntry() 
{
    ...
    current_col->SetHeader().SetTitle("Current Lat-Lon");
    ...
    {
        string suggested = CSubSource::FixLatLonFormat(*fi,true);
        if (suggested == *fi)
            continue;
...
CRef<CCmdComposite> CLatLonTool::GetCommandFromValuesTable(CRef<CSeq_table> values_table) 
{
    ...
    CRef<CCmdComposite> cmd(new CCmdComposite("LatLon Tool"));
    m_Current_to_Suggested.clear();
    m_Modified = false;
    for (size_t row = 0; row < values_table->GetColumn("current").GetData().GetString().size(); ++row)
    {
	    string current = values_table->GetColumn("current").GetData().GetString()[row];
	    string suggested = values_table->GetColumn("suggested").GetData().GetString()[row];
	    if (!current.empty() && !suggested.empty() && current != suggested && (m_Grid->IsInSelection(row,0) || select_all))
	        m_Current_to_Suggested[current] = suggested;
    }
...
CRef<CCmdComposite> CLatLonTool::GetCommand()
{
...
{
    return "Invalid operation in LatLon Tool";
}
...
{
    // Bitmap retrieval
////@begin CLatLonTool bitmap retrieval
    wxUnusedVar(name);
    return wxNullBitmap;
////@end CLatLonTool bitmap retrieval
}
...
{
    // Icon retrieval
////@begin CLatLonTool icon retrieval
    wxUnusedVar(name);
    return wxNullIcon;
////@end CLatLonTool icon retrieval
}
...
{
////@begin wxEVT_COMMAND_BUTTON_CLICKED event handler for ID_BUTTON in CLatLonTool.
    // Before editing this code, remove the block markers.
    event.Skip();
////@end wxEVT_COMMAND_BUTTON_CLICKED event handler for ID_BUTTON in CLatLonTool. 
}
...
void CLatLonTool::x_GatherLatLon(const CBioSource& biosource)
{
    FOR_EACH_SUBSOURCE_ON_BIOSOURCE(subsource, biosource)
    {
        if ((*subsource)->IsSetSubtype() && (*subsource)->GetSubtype() == CSubSource::eSubtype_lat_lon && (*subsource)->IsSetName())
        {
            string latlon = (*subsource)->GetName();
            if (!latlon.empty())
            {
...
bool CLatLonTool::x_ApplyToBioSource (CBioSource& biosource)
{       
    bool modified = false;
    EDIT_EACH_SUBSOURCE_ON_BIOSOURCE(subsource, biosource)
    {
        if ((*subsource)->IsSetSubtype() &&  (*subsource)->GetSubtype() == CSubSource::eSubtype_lat_lon && (*subsource)->IsSetName())
        {
            string latlon = (*subsource)->GetName();
            m_Found = true;
            if (m_Current_to_Suggested.find(latlon) != m_Current_to_Suggested.end())
            {
                (*subsource)->SetName(m_Current_to_Suggested[latlon]);
                modified = true;
            }
        }
    }
...
void CLatLonTool::x_ApplyToDescriptors(const CSeq_entry& se, CCmdComposite* composite)
{
    if (!composite) {
...
void CLatLonTool::x_FindBioSource(CSeq_entry_Handle tse, CCmdComposite* composite)
{
