/*  $Id: gff_export_job.cpp 37285 2016-12-21 17:56:47Z katargir $
* ===========================================================================
*
...
CGffExportJob::CGffExportJob(const CGffExportParams& params)
: CAppJob("GFF3 Export"), m_Params(params)
{
}

IAppJob::EJobState CGffExportJob::Run()
{
    string err_msg;
    ...
    try {
		...
		if (m_Params.GetExtraQuals())
			flags |= CGff3Writer::fExtraQuals;

        unsigned long depth;
        m_Params.GetFeatureDepth().ToULong(&depth);

		CGff3Writer writer(*scope, ostr, flags);
