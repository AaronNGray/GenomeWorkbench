/*  $Id: agp_exporter.cpp 39345 2017-09-13 19:39:22Z katargir $
* ===========================================================================
*
...
protected:
    ...
    CAgpExportPage  *m_Panel;
    string           m_RegPath;

    TRelatedObjectsMap  m_Objects;
    TConstScopedObjects m_Input;
...
CAgpExporter::CAgpExporter()
    : m_Descriptor("AGP File", ""),
    m_ParentWindow(),
    m_Panel(),
    m_State(eInvalid)
{
...
const IUIObject& CAgpExporter::GetDescriptor() const
{
    return m_Descriptor;
}

void CAgpExporter::SetServiceLocator(IServiceLocator* /*srv_locator*/)
{
}

void CAgpExporter::SetParentWindow(wxWindow* parent)
{
    m_ParentWindow = parent;
}

void CAgpExporter::InitUI()
{
    m_State = eParams;
}

void CAgpExporter::CleanUI()
{
    m_State = eInvalid;
    m_Panel = NULL;
}

wxPanel* CAgpExporter::GetCurrentPanel()
{
    if (m_Panel == NULL) {
        m_Panel = new CAgpExportPage(m_ParentWindow);
        m_Panel->SetData(m_Params);
        m_Panel->SetObjects( &m_Objects );
        //m_Panel->SetObjects( m_Input );
...
bool CAgpExporter::IsFinalState()
{
    return m_State == eParams;
}

bool CAgpExporter::IsCompletedState()
{
    return m_State == eCompleted;
}

bool CAgpExporter::CanDo(EAction action)
{
    switch (m_State)
    {
    case eInvalid:
...
bool CAgpExporter::DoTransition(EAction action)
{
...
IAppTask* CAgpExporter::GetTask()
{
    CIRef<IAppJob> job(new CAgpExportJob(m_Params));
    return new CAppExportTask(*job, m_Params.GetFileName());
}

void CAgpExporter::SetRegistryPath(const string& path)
{
    m_RegPath = path; // store for later use
    m_Params.SetRegistryPath(m_RegPath + ".AgpParams");
}

void CAgpExporter::SetLocations(const TRelatedObjectsMap& input_map)
{
    m_Objects = input_map;
}

void CAgpExporter::SetObjects( const TConstScopedObjects& input )
{
    m_Input = input;
...
void CAgpExporter::SaveSettings() const
{
    m_Params.SaveSettings();
    if (m_Panel) m_Panel->SaveSettings();
}

void CAgpExporter::LoadSettings()
{
    m_Params.LoadSettings();
}

string  CAgpExporterFactory::GetExtensionIdentifier() const
{
    static string sid("agp_exporter_factory");
    return sid;
}


string CAgpExporterFactory::GetExtensionLabel() const
{
    static string slabel("AGP Exporter Factory");
    return slabel;
}

IUIExportTool*  CAgpExporterFactory::CreateInstance() const
{
    return new CAgpExporter();
}

bool CAgpExporterFactory::TestInputObjects(TConstScopedObjects& objects) const
{
    ITERATE(TConstScopedObjects, it, objects) {
        const CSerialObject* so =
            dynamic_cast<const CSerialObject*>(it->object.GetPointer());

        string typeName = so->GetThisTypeInfo()->GetName();
        if (typeName == "BioTreeContainer")
            continue;
